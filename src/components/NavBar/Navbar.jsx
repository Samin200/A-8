  ...
  return (
    <div className=" flex justify-between text-[16px] items-center px-[1%] bg-white text-black py-[17.5px] relative">
      <div className="flex items-center gap-1">
        <div className="hidden max-sm:contents" onClick={()=> handleDrawer()}><FontAwesomeIcon icon={faBars} /></div>
        <div>
      ...
      <div className={`flex max-sm:flex-col items-center sm:gap-8 font-medium max-sm:absolute max-sm:hidden  ${drawer ? 'show-menu max-sm:bg-gray-300' : 'max-sm:hidden'}`}>
        <NavLink onClick={() => setCurrentClicked(1)} to={'/'} className={`hover-effect ${ currentClicked === 1 ? 'click-effect' : ''}`}>Home</NavLink>
        <NavLink onClick={() => setCurrentClicked(2)} to={'/Apps'} className={`hover-effect ${ currentClicked === 2 ? 'click-effect' : ''} `}>Apps</NavLink>
        <NavLink onClick={() => setCurrentClicked(3)} to={'/Install'} className={`hover-effect ${ currentClicked === 3 ? 'click-effect' : ''} `}>Installation</NavLink>
      </div>
